...
const Pricing = () => {
  ...
  const closeAndSaveModal = () => {
    // --- CONFIRMATION ALERT ---
    const isConfirmed = window.confirm("Are you sure you want to update this service? Changes will be applied immediately to the mobile app.");
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-3 md:p-8">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
        ...
        <button 
          ...
          className="w-full md:w-auto bg-[#2563EB] text-white px-6 py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden border border-gray-100">
            <div className="p-5 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
              <h3 className="text-sm font-black text-gray-800 uppercase tracking-tight">Service Configuration</h3>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">App Status</label>
                  <div className="w-full bg-green-50 border border-green-100 text-green-600 rounded-xl px-4 py-3 text-[10px] font-black text-center uppercase">Live</div>
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Target Details</label>
                <input 
                  type="text" 
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none"
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Inclusions</label>
                <textarea 
                  rows="3"
            ...
            <div className="p-6 bg-gray-50 flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={closeAndSaveModal} className="order-1 sm:order-2 flex-1 px-4 py-3 rounded-xl bg-blue-600 text-white text-xs font-bold hover:bg-blue-700">UPDATE SERVICE</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto mt-12 grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
             <div className="space-y-1.5">
               ...
               <div className="relative">
                 ...
                 <input type="number" value={fees.delivery_fee} onChange={e => setFees({...fees, delivery_fee: e.target.value})} className="w-full bg-gray-50 border-none rounded-xl pl-8 pr-4 py-3 text-sm font-black text-gray-700 outline-none"/>
               </div>
             </div>
             <div className="space-y-1.5">
               ...
               <div className="relative">
                 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">₱</span>
                 <input type="number" value={fees.heavy_garment_fee} onChange={e => setFees({...fees, heavy_garment_fee: e.target.value})} className="w-full bg-gray-50 border-none rounded-xl pl-8 pr-4 py-3 text-sm font-black text-gray-700 outline-none"/>
               </div>
             </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          <h3 className="text-xs font-black text-gray-400 uppercase mb-5 flex items-center gap-2"><Info size={14}/> Service Areas</h3>
          <textarea 
            value={serviceAreas} 
            onChange={e => setServiceAreas(e.target.value)}
            className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-xs font-medium resize-none min-h-[90px] outline-none" 
            placeholder="Mambaling, Basak San Nicolas..."
          />
        </div>
      </div>
